...
pub fn run_agent() -> Result<()> {
    ...
    // Freshness is the Bitwarden desktop app's job: it syncs its own data.json
    // while running (and it must be running for biometric unlock anyway), and
    // reads always re-read that file — so there's no sync for the agent to do.
